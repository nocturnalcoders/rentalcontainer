...
class Reqempty extends CI_Controller {
	...
	public function index()
	{
		if ($this->fungsi->user_login()->level == 2) { 
			...
			$data['row'] = $this->reqempty_m->getawal($divbran);   //panggil model dan functionnya
		  
		  } else
		  {
			$data['row'] = $this->reqempty_m->get();   //panggil model dan functionnya
		  }
		...
		$this->template->load('template','reqempty/reqempty_data', $data);  //loading template, feeder_data dan variable data
	}
 ...
 */
	...
	public function create()
	{
		...
		// print_r($post);
		// die();
		$url = $this->reqempty_m->add($post);
		$this->session->set_flashdata('success', 'Data Added Successfully');
		redirect('reqempty/edit/'.$url); 
	}

	public function cekrequest()
	{
		$count = $this->reqempty_m->cekrequest();
		echo $count;
	}
	...
	public function add()
	{
		...
		$this->template->load('template','reqempty/reqempty_form' , $data);  //mau sebagai tambah sekaligus edit, perlu lempar data
	}
	
	public function edit($id)
	{
		$query = $this->reqempty_m->get(str_replace("@","/",$id));

		if($query->num_rows() >0) {
			$feeder = $query->row();
			$detailQuery = $this->reqempty_m->getDetail(str_replace("@","/",$id));
			...
			$portdestQuery = $this->master_m->getPort();
			$data['portdestRow'] = $portdestQuery->result();
			$this->template->load('template','reqempty/reqempty_formedit' , $data);  //mau sebagai tambah sekaligus edit, perlu lempar data
		} else {
			echo "<script>alert('Data Not Found');</script>";
			echo "<script>window.location='".site_url('reqempty')."';</script>";
		}
	}
	...
	public function process()
	{
        ...
        redirect('reqempty/edit/'.$requestid);
    }

    public function adddetail()
	{
		$post = $this->input->post(null, TRUE);
		$this->reqempty_m->adddetail($post);
    ...
    public function updatedetail()
	{
		$post=$this->input->post(null, TRUE);
		// die(print_r($post));
		$this->reqempty_m->editDetail($post);
	...
	{
		$post=$this->input->post(null, TRUE);
		$this->reqempty_m->hapusDetail($post);
        echo "sukses++Record Deleted";
    }

	public function del($id)
	{
		$this->reqempty_m->del($id);
		if($this->db->affected_rows() > 0) 
		{
	...
	{
		$post=$this->input->post(null, TRUE);
		$data['row'] = $this->reqempty_m->cari($post);
		$data['search'] = $post;
		$divbranQuery = $this->divbran_m->get();
		$data['divbranRow'] = $divbranQuery->result();
		$portdestQuery = $this->master_m->getPort();
		$data['portdestRow'] = $portdestQuery->result();
		$this->template->load('template','reqempty/reqempty_data', $data);
	}
