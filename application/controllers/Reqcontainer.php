...
class Reqcontainer extends CI_Controller {
	...
	function __construct()    //digunakan untuk meload model pertama kali
	{
		...
		check_not_login();
		check_admin();
		$this->load->model('reqcontainer_m'); 
	...
	public function index()
	{
		if ($this->fungsi->user_login()->level == 1) { 
			...
			$data['row'] = $this->reqcontainer_m->getawal($divbran);   //panggil model dan functionnya
		  
		  } else
		  {
			$data['row'] = $this->reqcontainer_m->get();   //panggil model dan functionnya
		  }
		...
		$this->template->load('template','reqcontainer/reqcontainer_data', $data);  //loading template, feeder_data dan variable data
	}
 ...
 */
	...
	public function create()
	{
		...
		// print_r($post);
		// die();
		$url = $this->reqcontainer_m->add($post);
		$this->session->set_flashdata('success', 'Data Added Successfully');
		redirect('reqcontainer/edit/'.$url); 
	}

	public function cekrequest()
	{
		$count = $this->reqcontainer_m->cekrequest();
		echo $count;
	}
	...
	public function add()
	{
			...
			$feeder->name = null ; 
			$type = $this->reqcontainer_m->getType();
		...
		$this->template->load('template','reqcontainer/reqcontainer_form' , $data);  //mau sebagai tambah sekaligus edit, perlu lempar data
	}
	
	public function edit($id)
	{
		$query = $this->reqcontainer_m->get(str_replace("@","/",$id));

		if($query->num_rows() >0) {
			$feeder = $query->row();
			$detailQuery = $this->reqcontainer_m->getDetail(str_replace("@","/",$id));
			...
			$portdestQuery = $this->master_m->getPort();
			$data['portdestRow'] = $portdestQuery->result();
			$this->template->load('template','reqcontainer/reqcontainer_formedit' , $data);  //mau sebagai tambah sekaligus edit, perlu lempar data
		} else {
			echo "<script>alert('Data Not Found');</script>";
			echo "<script>window.location='".site_url('reqcontainer')."';</script>";
		}
	}
	...
	public function process()
	{
        ...
        redirect('reqcontainer/edit/'.$requestid);
    }

    public function adddetail()
	{
		$post = $this->input->post(null, TRUE);
		$this->reqcontainer_m->adddetail($post);
    ...
    public function updatedetail()
	{
		$post=$this->input->post(null, TRUE);
		// die(print_r($post));
		$this->reqcontainer_m->editDetail($post);
	...
	{
		$post=$this->input->post(null, TRUE);
		$this->reqcontainer_m->hapusDetail($post);
        echo "sukses++Record Deleted";
    }

	public function del($id)
	{
		$this->reqcontainer_m->del($id);
		if($this->db->affected_rows() > 0) 
		{
	...
	{
		$post=$this->input->post(null, TRUE);
		$data['row'] = $this->reqcontainer_m->cari($post);
		$data['search'] = $post;
		$divbranQuery = $this->divbran_m->get();
		$data['divbranRow'] = $divbranQuery->result();
		$portdestQuery = $this->master_m->getPort();
		$data['portdestRow'] = $portdestQuery->result();
		$this->template->load('template','reqcontainer/reqcontainer_data', $data);
	}
